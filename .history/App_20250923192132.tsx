...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ className = '', onAnimationComplete, scrolled = false }) => {
  ...
  return (
    <div className={`relative w-32 h-12 ${className}`}>
      {/* Logo original - Siempre visible */}
      <div className="absolute inset-0 flex items-center justify-center">
        <img 
          src="Assets/pruebasvg.svg" 
          alt="ARQUIMEC Logo" 
          className="w-20 h-20 object-contain drop-shadow-lg"
          onError={(e) => {
            console.error('Error loading logo:', e);
            // Fallback al texto si el SVG no carga
            setShowLogo(false);
          }}
        />
      </div>

      {/* Texto ARQUIMEC - Solo se muestra cuando showLogo es false */}
      <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${
        !showLogo ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
      }`}>
        <span className={`text-lg font-thin tracking-wider drop-shadow-lg animate-pulse ${
      ...
      >
        <div className="w-full px-2 lg:px-2">
          <div className="flex justify-between items-center py-4 lg:py-6">
            ...
            >
              <AnimatedLogo className="h-8 lg:h-10 w-auto" scrolled={scrolled} />
            </motion.div>
              ...
              >
                <Button
                  ...
                  className={`border-2 uppercase px-6 py-3 text-sm tracking-wider transition-all duration-500 transform hover:scale-101  ${
                    scrolled
                      ? "border-red-500  text-black hover:text-amber-500 hover:border-gray-800"
