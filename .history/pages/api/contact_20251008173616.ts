...
export const config = {
  api: {
    bodyParser: false, // Deshabilitamos el bodyParser por defecto
  },
};
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const mensaje = Array.isArray(fields.mensaje) ? fields.mensaje[0] : fields.mensaje;
    
    // Manejar archivo si existe
    const archivo: File | undefined = files.archivo && Array.isArray(files.archivo) ? files.archivo[0] : files.archivo;
        ...
        <div style="font-family: 'Montserrat', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #ffffff;">
          ...
          <div style="background-color: #e3f2fd; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #bbdefb;">
            <p style="margin: 0; color: #1565c0; font-weight: 500;">
              📎 Archivo adjunto: ${archivo.originalFilename} (${(archivo.size / 1024 / 1024).toFixed(2)} MB)
            </p>
          </div>
          ` : ''}
