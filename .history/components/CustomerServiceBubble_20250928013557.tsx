...
const CustomerServiceBubble: React.FC<CustomerServiceBubbleProps> = ({ 
  ...
  const generateAIResponse = (userMessage: string, context: typeof conversationContext) => {
    ...
    const keywords = {
      ...
      residencial: ['casa', 'hogar', 'vivienda', 'residencial', 'familia', 'unifamiliar', 'complejo', 'residencia', 'departamento'],
      comercial: ['comercial', 'oficina', 'retail', 'restaurante', 'negocio', 'corporativo', 'empresarial'],
      interiores: ['interior', 'decoración', 'ambiente', 'muebles', 'colores', 'estilo', 'diseño interior', 'interiores', 'rediseñar', 'rediseño', 'departamento'],
    ...
    Object.entries(keywords).forEach(([intent, words]) => {
      ...
      // Buscar coincidencias exactas de frases completas primero
      words.forEach(word => {
      ...
      const individualWords = message.split(' ');
    ...
    // Verificar si hay palabras clave específicas para empresa
    const empresaKeywords = ['quienes', 'somos', 'son', 'empresa', 'estudio', 'arquimec', 'información', 'saber'];
    const hasEmpresaKeywords = empresaKeywords.some(keyword => message.includes(keyword));
    
    // Detectar consultas específicas sobre servicios
    const serviceKeywords = ['diseño de interior', 'interiores', 'rediseñar', 'departamento', 'ambiente'];
    const hasServiceKeywords = serviceKeywords.some(keyword => message.includes(keyword));
    
    // Solo responder con información de empresa si es una pregunta directa sobre la empresa
    if (hasEmpresaKeywords && message.includes('quienes') && (detectedIntent === 'empresa' || confidence === 0)) {
      response = getContextualResponse('empresa') || defaultResponses[0];
    } else if (hasServiceKeywords && (detectedIntent === 'interiores' || message.includes('interior'))) {
      response = getContextualResponse('interiores') || defaultResponses[0];
    } else if (detectedIntent && confidence > 0) {
      ...
      // Respuestas más conversacionales para mantener el flujo
      const conversationalResponses = [
        "Entiendo, cuéntame más detalles sobre tu proyecto. ¿Qué tipo de espacio necesitas rediseñar?",
        "Perfecto, me interesa conocer más sobre tu proyecto. ¿Podrías darme más información?",
        "Excelente, ¿qué aspectos específicos te gustaría mejorar en tu espacio?",
        "Interesante, ¿hay algo en particular que te gustaría cambiar o mejorar?"
      ];
