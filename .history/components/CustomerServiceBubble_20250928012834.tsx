...
const CustomerServiceBubble: React.FC<CustomerServiceBubbleProps> = ({ 
  ...
  const generateAIResponse = (userMessage: string, context: typeof conversationContext) => {
    ...
    const keywords = {
      empresa: ['quienes somos', 'empresa', 'estudio', 'arquimec', 'información', 'historia', 'filosofía'],
      servicios: ['servicio', 'contratar', 'precio', 'costo', 'tarifa', 'presupuesto', 'qué hacen', 'ofrecen'],
      residencial: ['casa', 'hogar', 'vivienda', 'residencial', 'familia', 'unifamiliar', 'complejo'],
      comercial: ['comercial', 'oficina', 'retail', 'restaurante', 'negocio', 'empresa', 'corporativo'],
      interiores: ['interior', 'decoración', 'ambiente', 'muebles', 'colores', 'estilo', 'diseño interior'],
      consultorias: ['consultoría', 'técnico', 'asesoramiento', 'normativa', 'reglamentación', 'permisos'],
      proceso: ['proceso', 'cómo trabajan', 'metodología', 'etapas', 'tiempo', 'duración'],
      contacto: ['contacto', 'teléfono', 'email', 'dirección', 'ubicación', 'oficina', 'dónde están'],
      urgencia: ['urgente', 'rápido', 'inmediato', 'ya', 'ahora', 'pronto']
    };
    ...
    const responses = {
      arquitectura: [
        "Entiendo que te interesa el diseño arquitectónico. ¿Tienes algún tipo de proyecto específico en mente?",
        "Perfecto, puedo ayudarte con información sobre nuestros servicios de arquitectura. ¿Es para una vivienda, comercial o industrial?",
        "Excelente elección. Nuestros arquitectos tienen experiencia en diversos tipos de proyectos. ¿Qué tipo de espacio necesitas diseñar?"
      ],
      servicios: [
        "Te puedo ayudar con información sobre nuestros servicios. ¿Te interesa conocer nuestros paquetes de diseño?",
        "Perfecto, puedo darte detalles sobre nuestros servicios. ¿Prefieres que te envíe información por WhatsApp?",
        "Excelente, tenemos varios servicios disponibles. ¿Te gustaría que un especialista te contacte directamente?"
      ],
      tecnico: [
        "Entiendo que necesitas información técnica. ¿Hay algún aspecto específico que te preocupa?",
        "Perfecto, puedo conectarte con nuestros ingenieros para consultas técnicas. ¿Es sobre estructura, instalaciones o materiales?",
        "Excelente, nuestros técnicos pueden ayudarte. ¿Qué tipo de consulta técnica tienes?"
      ],
      contacto: [
        "Te puedo proporcionar nuestros datos de contacto. ¿Prefieres WhatsApp, email o teléfono?",
        "Perfecto, te puedo dar toda nuestra información de contacto. ¿Te parece bien que te la envíe por WhatsApp?",
        "Excelente, puedo darte nuestros datos para que nos contactes directamente."
      ],
      urgencia: [
        "Entiendo la urgencia. Te puedo conectar inmediatamente con nuestro equipo. ¿Prefieres WhatsApp para respuesta rápida?",
        "Perfecto, para casos urgentes te recomiendo contactarnos por WhatsApp. ¿Te parece bien?",
        "Entiendo, para asuntos urgentes es mejor el contacto directo. ¿Te conecto con nuestro equipo?"
      ]
    };

    // Respuesta por defecto si no se detecta intención específica
    const defaultResponses = [
      "Interesante, cuéntame más sobre lo que necesitas.",
      "Entiendo, ¿podrías darme más detalles sobre tu consulta?",
      "Perfecto, ¿hay algo específico en lo que pueda ayudarte?",
      "Excelente, ¿qué más te gustaría saber sobre nuestros servicios?"
    ];

    // Seleccionar respuesta basada en la intención detectada
    let response = '';
    if (detectedIntent && responses[detectedIntent as keyof typeof responses]) {
      const intentResponses = responses[detectedIntent as keyof typeof responses];
      response = intentResponses[Math.floor(Math.random() * intentResponses.length)];
    } else {
  ...
  const handleConversationAction = (action: 'continue' | 'end' | 'whatsapp') => {
    switch (action) {
      ...
      case 'end':
        ...
        break;
      case 'whatsapp':
        const conversationSummary = `Resumen de conversación con ${formData.name} (${formData.email}):\n\n` +
          `Consulta: ${conversationContext.selectedOption}\n` +
          `Resumen: ${conversationContext.conversationHistory.join(' ')}\n\n` +
          `Cliente interesado en continuar conversación por WhatsApp.`;
        
        const encodedMessage = encodeURIComponent(conversationSummary);
        const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
        window.open(whatsappUrl, '_blank', 'noopener,noreferrer');
        addMessage("Te he enviado un resumen de nuestra conversación por WhatsApp. Un especialista se pondrá en contacto contigo pronto.", false);
        break;
    }
  };
