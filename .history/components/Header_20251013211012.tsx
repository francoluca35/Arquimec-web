...
const Header: React.FC<HeaderProps> = ({ scrolled, headerVisible }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const menuButton = target.closest('button[aria-controls="mobile-menu"]');
      
      if (mobileMenuOpen && mobileMenu && !mobileMenu.contains(target) && !menuButton) {
          ...
          <div className="hidden lg:flex items-center space-x-6 pr-4">
            <nav className="flex items-center space-x-4">
              {[
                ...
                { name: "CONSTRUCTORA", href: "#estudio" },
                ...
                item.href.startsWith('/') ? (
                  <motion.button
                    ...
                    className={`transition-colors duration-500 text-sm tracking-wider underline-offset-4 hover:underline ${
                      scrolled 
                        ? "text-black hover:text-[#b48f42]" 
                        : "text-white hover:text-[#b48f42]"
                    }`}
                    style={{
                      fontWeight: 400,
                      letterSpacing: "0.1em",
          ...
          <div className="lg:hidden pr-4">
            <button
              ...
              className={`p-2 rounded-md transition-colors duration-200 ${
                scrolled 
                  ? 'text-gray-700 hover:bg-[#e4e4e4]' 
                  : 'text-white hover:bg-[#1a2a3c]'
              }`}
