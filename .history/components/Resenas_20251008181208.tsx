...
const Resenas: React.FC = () => {
  ...
  // Combinar reseñas estáticas con las de Firebase
  const todasLasReseñas = [...reseñasFirebase.map(r => ({
    id: r.id,
    nombre: r.nombre,
    ubicacion: r.lugar,
    rating: r.calidad,
  ...
  const totalSlides = Math.ceil(todasLasReseñas.length / 2);
              ...
              {loading ? (
                ...
                Array.from({ length: totalSlides }, (_, slideIndex) => (
                  <div key={slideIndex} className="w-full flex-shrink-0 px-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {todasLasReseñas.slice(slideIndex * 2, slideIndex * 2 + 2).map((reseña, index) => (
                      <motion.div
                        key={reseña.id}
