  ...
  const advancedAI = {
    ...
    getInitialResponse: (intent: string, message: string) => {
      const responses = {
        ...
        interiores: this.getInteriorResponse(message),
    ...
    getExploringResponse: (intent: string, message: string, context: typeof conversationContext) => {
      ...
      if (intent === 'interiores' || context.lastIntent === 'interiores') {
        if (hasAge && hasDimensions) {
          ...
          return this.getSeniorFriendlyResponse(age, dimensions);
        } else if (hasIdeas) {
          return this.getIdeasResponse(message);
        } else if (hasAge) {
          return this.getAgeSpecificResponse(hasAge[1]);
        } else if (hasDimensions) {
          return this.getDimensionSpecificResponse(hasDimensions[1]);
        }
      }
