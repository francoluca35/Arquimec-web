...
const AIChatbot: React.FC<AIChatbotProps> = ({ isOpen, onClose }) => {
  ...
  const callOpenAI = async (userMessage: string, conversationHistory: Message[]): Promise<string> => {
    try {
      ...
      return chatbotResponses.fallback.responses[Math.floor(Math.random() * chatbotResponses.fallback.responses.length)];
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (jsonResponse) {
        ...
        setIsLoading(true);
        aiResponse = await callOpenAI(userMessage, messages);
        setIsLoading(false);
      }
