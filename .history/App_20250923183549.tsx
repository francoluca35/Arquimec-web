...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ className = '', onAnimationComplete }) => {
  const [animationPhase, setAnimationPhase] = useState<'forming' | 'formed' | 'disassembling' | 'reforming'>('forming');
  const [particles, setParticles] = useState<Array<{id: number, x: number, y: number, vx: number, vy: number, size: number, color: string}>>([]);

  // Generar partículas para el desarme
  const generateParticles = () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      newParticles.push({
        id: i,
        x: 20, // Centro del logo pequeño
        y: 20,
        vx: Math.cos(angle) * (Math.random() * 3 + 2),
        vy: Math.sin(angle) * (Math.random() * 3 + 2),
        size: Math.random() * 3 + 1,
        color: Math.random() > 0.3 ? '#FFD700' : Math.random() > 0.5 ? '#FFA500' : '#FF8C00'
      });
    }
  ...
  useEffect(() => {
    ...
    const timer3 = setTimeout(() => {
      setAnimationPhase('reforming');
    }, 3000);

    const timer4 = setTimeout(() => {
      setAnimationPhase('forming');
      // Reiniciar el ciclo infinito
    }, 4000);
  ...
  }, [animationPhase]); // Dependencia en animationPhase para reiniciar
  ...
  return (
    <div className={`relative w-12 h-12 ${className}`}>
      ...
      <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${
        ...
        animationPhase === 'disassembling' ? 'opacity-0 scale-0' :
        animationPhase === 'reforming' ? 'opacity-100 scale-100' :
          ...
          {particles.map((particle, index) => (
            <div
              ...
              style={{
                ...
                transform: `translate(${particle.vx * 30}px, ${particle.vy * 30}px) scale(${1 - index * 0.01})`,
      ...
      {animationPhase === 'reforming' && (
        <div className="absolute inset-0 flex items-center justify-center">
          {/* Círculos concéntricos girando */}
          <div className="w-80 h-80 border-4 border-yellow-400 rounded-full animate-spin opacity-60"></div>
          <div className="absolute w-64 h-64 border-3 border-yellow-300 rounded-full animate-spin opacity-70" style={{animationDirection: 'reverse', animationDuration: '1.5s'}}></div>
          <div className="absolute w-48 h-48 border-2 border-yellow-200 rounded-full animate-spin opacity-80" style={{animationDuration: '2s'}}></div>
          
          {/* Partículas que regresan */}
          <div className="absolute inset-0">
            {particles.slice(0, 20).map((particle, index) => (
              <div
                key={`return-${particle.id}`}
                className="absolute rounded-full"
                style={{
                  left: (particle.x + particle.vx * 30) + 'px',
                  top: (particle.y + particle.vy * 30) + 'px',
                  width: particle.size + 'px',
                  height: particle.size + 'px',
                  backgroundColor: particle.color,
      ...
      <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${
        ...
        <h1 className="text-5xl font-bold bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 bg-clip-text text-transparent animate-pulse drop-shadow-lg">
          ARQUIMEC
        </h1>
      </div>
    </div>
      ...
      >
        <div className="w-full px-2 lg:px-2">
          <div className="flex justify-between items-center py-4 lg:py-6">
            ...
            {/* Desktop Navigation - Pegado a la derecha */}
            <div className="hidden lg:flex items-center space-x-6 pr-4">
              ...
              >
                <Button
                  variant="outline"
                  className={`border-2 uppercase px-6 py-3 text-sm tracking-wider transition-all duration-500 transform hover:scale-101  ${
                    scrolled
                      ? "border-black text-black hover:text-yellow-400 hover:border-yellow-400"
            ...
            >
              {mobileMenuOpen ? (
                <X size={24} className={`transition-colors duration-500 ${
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="flex flex-col space-y-4">
                ...
                <Button
                  variant="outline"
                  className={`border-2 w-full mt-4 text-sm tracking-wider transition-all duration-500 transform hover:scale-101 hover:-translate-y-1 ${
                    scrolled
                      ? "border-black text-black hover:text-yellow-400 hover:border-yellow-400"
                      : "border-white text-white hover:text-yellow-400 hover:border-yellow-400"
                  }`}
