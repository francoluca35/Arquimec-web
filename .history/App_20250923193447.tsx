...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ className = '', onAnimationComplete, scrolled = false }) => {
  ...
  return (
    <div className={`relative w-32 h-12 ml-10 ${className}`}>
      {/* Logo con círculo y letra A */}
      <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${
        showLogo ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
      }`}>
        <div className="relative w-12 h-12 flex items-center justify-center">
          {/* Círculo de fondo */}
          <div className="absolute inset-0 rounded-full border-2 border-white bg-[#1a2a3c] flex items-center justify-center">
            {/* Letra A */}
      ...
      <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${
        ...
        <span className={`text-lg font-bold tracking-wider drop-shadow-lg ${
